...
########################################1. 데이터 파일 합치기#################################################
# Combine train and test datasets for PM2.5 data
...
# Combine train and test datasets for AWS data
train_aws = pd.concat([pd.read_csv(file) for file in train_aws_files], ignore_index=True)
test_aws = pd.concat([pd.read_csv(file) for file in test_aws_files], ignore_index=True)

# Find the unique set of locations in both the train and test datasets
location_set = set(train_data['측정소']).intersection(set(test_data['측정소']))
aws_location_set = set(train_aws['지점']).intersection(set(test_aws['지점']))

# Apply label encoding only to the locations present in both train and test datasets
le = LabelEncoder()
le2 = LabelEncoder()
train_data = train_data[train_data['측정소'].isin(location_set)]
train_data['location'] = le.fit_transform(train_data['측정소'])
test_data = test_data[test_data['측정소'].isin(location_set)]
test_data['location'] = le.transform(test_data['측정소'])

train_aws = train_aws[train_aws['지점'].isin(aws_location_set)]
train_aws['location'] = le2.fit_transform(train_aws['지점'])
test_aws = test_aws[test_aws['지점'].isin(aws_location_set)]
test_aws['location'] = le2.transform(test_aws['지점'])
print(test_data)
print(test_aws) #[93600 rows x 9 columns]
